# Skills：教育信息化治理研究技能导读

本目录是 `Ed-PhD-Copilot-Skills` 的可执行核心。它参考 `Supervisor-Skills` 的技能化组织方式，但把能力中心从计算机顶会论文转向教育技术学、教育领导与管理、教育信息化治理方向。

## 推荐使用主线

### 主线一：治理选题到论文骨架

1. `governance-idea-evaluator`：先判断选题是否具有政策价值、组织可行性、理论贡献和伦理可控性。
...
`ai-assisted-edtech-research-workflow` 可贯穿任一阶段，用于划分研究者与 AI 的分工边界，防止 AI 代替选题判断、理论判断、证据判断和学术责任。

## 技能卡片

| Skill | 触发场景 | 主要产出 |
|---|---|---|
| `governance-idea-evaluator` | 评估智能校园、教育数据治理、AI 教育治理、教师数字胜任力等选题 | 选题体检表、风险等级、推进/转向建议 |
| `edtech-intro-drafter` | 起草教育技术/教育治理论文 Introduction | 四重奏引言大纲、逻辑缺口、贡献落点 |
| `governance-paper-template` | 规划系统架构、机制阐释、治理模式类论文 | 论文骨架、理论架桥、双层拓扑、实践进路 |
| `mixed-methods-evidence-template` | 规划指标构建、评价模型、准实验或混合研究 | 指标论证、信效度方案、三角互证证据链 |
...
| `mixed-methods-evidence-template` | `claim-evidence-matrix.md`, `indicator-system.md`, `quantitative-methods.md`, `qualitative-methods.md`, `triangulation.md`, `method-reporting-templates.md`, `design-science-reporting.md` |
...
V1.5 起，`mixed-methods-evidence-template` 和 `edtech-pre-submission-reviewer` 已加入报告规范与证据边界检查：

- 系统综述/证据地图：使用 PRISMA 风格的检索、筛选、纳排和流程记录要求。
- 访谈/焦点小组：使用 COREQ 风格的研究者角色、抽样、访谈协议、编码和引文支持要求。
