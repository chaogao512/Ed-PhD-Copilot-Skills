# Notice and Attribution

This project is an education-technology and education-governance adaptation project inspired by:

- `HKUSTDial/Supervisor-Skills`
- Repository: https://github.com/HKUSTDial/Supervisor-Skills
- Original license: Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0)

`Ed-PdD-Copilot-Skills` borrows the high-level idea of organizing research guidance as a dual-track "Handbook + Skills" repository and adapts that idea for educational technology, educational leadership and management, and education informatization governance.

The current project rewrites the skill content for the following direction:

- education informatization governance
- smart campus governance
- education data governance
- AI governance in education
- human-AI collaboration in educational organizations
- mixed-methods evidence design for educational technology research

Unless a file states otherwise, this project is released under CC BY-NC-SA 4.0.

When reusing or adapting this repository, please credit both this project and the original `Supervisor-Skills` project where appropriate.
