...
## V1.7 Full-Chain Requirements

真实博士选题的全链路压力测试必须体现：

- 7 个技能全部运行，且输出之间不发生研究对象漂移。
- `edtech-intro-drafter` 必须把政策背景、技术可能、治理异化和研究合法性串成论证链。
- `governance-figure-designer` 至少给出一个可渲染 artifact 或明确指向已有 artifact。
- `ai-assisted-edtech-research-workflow` 必须包含 AI 使用披露、保密边界和人工责任。
- 博士阶段输出应能映射到开题、中期、预答辩和正式答辩。
